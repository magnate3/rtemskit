...
({ \
	uint64_t __start = rtems_counter_ticks_to_nanoseconds(rtems_counter_read()); \
	uint64_t __timeout_ns = (timeout_ms) * 1000000ul; \
	int __ret; \
	for (;;) { \
		...
		if (__timeout_ns && \
		    rtems_counter_difference(rtems_counter_read(), __start) > __timeout_ns) { \
			__ret = regmap_read((map), (addr), &(val)); \
			break; \
		} \
		if ((sleep_us)) \
