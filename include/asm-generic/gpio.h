...
struct gpio_irqchip {
   const char  *name;
   void (*irq_mask)(struct gpio_desc *desc);
   void (*irq_unmask)(struct gpio_desc *desc);
   int  (*irq_set_type)(struct gpio_desc *desc, 
...
struct gpio_dev_priv {
	...
	const struct gpio_irqchip *irqchip;
	struct gpio_irqdesc **irqdescs;
...
static inline void gpio_device_lock(struct udevice *dev, 
	rtems_interrupt_lock_context *ctx)
{
	struct gpio_dev_priv *uc_priv = dev_get_uclass_priv(dev);
	rtems_interrupt_lock_acquire(&uc_priv->lock, ctx);
	(void) uc_priv;
}

static inline void gpio_device_unlock(struct udevice *dev,
	rtems_interrupt_lock_context *ctx)
{
	struct gpio_dev_priv *uc_priv = dev_get_uclass_priv(dev);
	rtems_interrupt_lock_release(&uc_priv->lock, ctx);
	(void) uc_priv;
}
 ...
 */
static inline void gpio_irq_handler(struct udevice *dev, int offset)
{
   struct gpio_dev_priv *uc_priv = dev_get_uclass_priv(dev);
   struct gpio_irqdesc *irqdesc;

   irqdesc = uc_priv->irqdescs[offset];
   do {
       irqdesc->handler(irqdesc->arg);
       irqdesc = irqdesc->next;
   } while (irqdesc);
}
