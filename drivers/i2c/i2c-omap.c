...
struct omap_i2c_bus_platform_data {
	...
	u32		rev;
	u32		flags;
	void    (*set_mpu_wkup_lat)(struct udevice *dev, long set);
};
...
struct omap_i2c_dev {
	...
	void			(*set_mpu_wkup_lat)(struct udevice *dev,
						    long latency);
...
static int omap_i2c_bus_speed(struct udevice *bus, unsigned int speed)
{
    struct omap_i2c_dev *omap = dev_get_priv(bus);
 ...
 */
static int omap_i2c_xfer_msg(struct omap_i2c_dev *omap, struct i2c_msg *msg, int stop)
{
	unsigned long timeout;
	u16 w;
 ...
 */
static int
omap_i2c_xfer(struct udevice *bus, struct i2c_msg msgs[], int num)
{
	struct omap_i2c_dev *omap = dev_get_priv(bus);
	int i;
	int r;
	...
	for (i = 0; i < num; i++) {
		r = omap_i2c_xfer_msg(omap, &msgs[i], (i == (num - 1)));
		if (r != 0)
			break;
	}
...
static void omap_i2c_isr(void *dev_id)
{
	struct omap_i2c_dev *omap = dev_id;
	u16 mask;
	u16 stat;
	...
	if (stat & mask) {
        int ret = omap_i2c_xfer_data(omap);
        if (ret != -EAGAIN)
...
static const struct udevice_id omap_i2c_of_match[] = {
	{
		.compatible = "ti,omap4-i2c",
		.data = (ulong)&omap4_pdata,
	},
	{
		.compatible = "ti,omap3-i2c",
		.data = (ulong)&omap3_pdata,
	},
	{
		.compatible = "ti,omap2430-i2c",
		.data = (ulong)&omap2430_pdata,
	},
	{
		.compatible = "ti,omap2420-i2c",
		.data = (ulong)&omap2420_pdata,
	},
	{ },
};
...
static int omap_i2c_of_to_plat(struct udevice *dev)
{
    struct omap_i2c_dev	*omap = dev_get_priv(dev);
    fdt_addr_t addr, irq;
	...
	omap->base = (void *)addr;
    omap->irq = irq;
	omap->speed = dev_read_u32_default(dev, "clock-frequency",
					   I2C_SPEED_STANDARD_RATE);
    return 0;
}

static int omap_i2c_probe(struct udevice *pdev)
{
	struct omap_i2c_dev	*omap = dev_get_priv(pdev);
    struct omap_i2c_bus_platform_data *pdata; 
	int r;
	u32 rev;
	u16 minor, major;

	pdata = (struct omap_i2c_bus_platform_data *)dev_get_driver_data(pdev);
	...
	if (omap->rev < OMAP_I2C_OMAP1_REV_2) {
		r = devm_request_threaded_irq(pdev, omap->irq, 
                  NULL, omap_i2c_omap1_isr, 
				  0, NULL, omap);
	} else {
		r = devm_request_threaded_irq(pdev, omap->irq, 
                  NULL, omap_i2c_isr, 
	...
	if (r) {
		dev_err(omap->dev, "failure requesting irq %i: %d\n", 
			omap->irq, r);
		goto err_unuse_clocks;
	}

	dev_info(omap->dev, "bus rev%d.%d at %d kHz\n", 
...
static int omap_i2c_remove(struct udevice *dev)
{
	struct omap_i2c_dev	*omap = dev_get_priv(dev);
...
static const struct dm_i2c_ops omap_i2c_ops = {
	.xfer		= omap_i2c_xfer,
	.set_bus_speed	= omap_i2c_bus_speed,
};

DM_DRIVER(i2c_omap) = {
	.name = "i2c_omap",
	.id = UCLASS_I2C,
	.of_match = of_match_ptr(omap_i2c_of_match),
    .probe = omap_i2c_probe,
    .remove = omap_i2c_remove,
    .of_to_plat = omap_i2c_of_to_plat,
    .priv_auto = sizeof(struct omap_i2c_dev),
    .ops = &omap_i2c_ops
};
