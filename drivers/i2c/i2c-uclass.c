...
int dm_i2c_xfer(struct udevice *bus, struct i2c_msg *msg, int nmsgs)
{
    ...
    rtems_mutex_lock(&i2c->lock);
	ret = ops->xfer(bus, msg, nmsgs);
    rtems_mutex_unlock(&i2c->lock);
    return ret;
}
			     ...
			     uint8_t *buffer, int len)
{
	...
	for (i = 0; i < len; i++) {
		...
		ret = dm_i2c_xfer(bus, msg, ptr - msg);
		if (ret)
			return ret;
	}
...
int dm_i2c_read(struct udevice *dev, uint offset, uint8_t *buffer, int len)
{
	...
	return dm_i2c_xfer(bus, msg, msg_count);
}
 ...
 * @return 0 if found, -ENOSYS if the driver is invalid, -EINVAL if the chip
...
static int i2c_pre_probe(struct udevice *dev)
{
    ...
    rtems_mutex_init(&i2c->lock, dev->name);
...
static int i2c_child_post_bind(struct udevice *dev)
{
	...
	rtems_mutex_init(&plat->lock, dev->name);
...
DM_DRIVER(i2c_generic_chip_drv) = {
