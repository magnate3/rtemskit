...
#define ARCH_DMA_MINALIGN CPU_ALIGNMENT
#endif
			      ...
			      uint of_plat_size, struct udevice **devp)
{
	...
	dev = rtems_calloc(1, sizeof(struct udevice));
	if (!dev)
		return -ENOMEM;
...
static void *alloc_priv(int size, uint flags)
{
	...
	if (flags & DM_FLAG_ALLOC_PRIV_DMA) {
		...
		priv = rtems_cache_coherent_allocate(size, ARCH_DMA_MINALIGN, 0);
		if (priv) {
			memset(priv, '\0', size);
			...
			rtems_cache_flush_multiple_data_lines(priv, size);
#endif
		}
	} else {
		priv = rtems_calloc(1, size);
	}
...
static int device_find_by_ofnode(ofnode node, struct udevice **devp)
{
	...
	list_for_each_entry(uc, _uclass_root, sibling_node) {
...
{
	*devp = _device_find_global_by_ofnode(_dm_root, ofnode);
...
int device_get_global_by_ofnode(ofnode ofnode, struct udevice **devp)
{
	...
	dev = _device_find_global_by_ofnode(_dm_root, ofnode);
