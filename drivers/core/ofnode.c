			  ...
			  u32 *outp)
{
	...
	cell = fdt_getprop(dm_fdt_blob(), ofnode_to_offset(node), propname,
			   &len);
	if (!cell) {
		debug("(not found)\n");
		return -EINVAL;
...
const void *ofnode_read_prop(ofnode node, const char *propname, int *sizep)
{
	...
	if (ofnode_is_np(node)) {
		...
		val = fdt_getprop(dm_fdt_blob(), ofnode_to_offset(node),
				  propname, &len);
	}
	if (!val) {
		debug("<not found>\n");
...
ofnode ofnode_find_subnode(ofnode node, const char *subnode_name)
{
	...
	if (ofnode_is_np(node)) {
		...
		int ooffset = fdt_subnode_offset(dm_fdt_blob(),
			  ...
			  u32 *out_values, size_t sz)
{
	...
	if (ofnode_is_np(node)) {
		...
		return fdtdec_get_int_array(dm_fdt_blob(),
...
bool ofnode_is_enabled(ofnode node)
{
	if (ofnode_is_np(node)) {
		...
		return fdtdec_get_is_enabled(dm_fdt_blob(),
...
ofnode ofnode_first_subnode(ofnode node)
{
	...
	return offset_to_ofnode(
		fdt_first_subnode(dm_fdt_blob(), ofnode_to_offset(node)));
}
...
ofnode ofnode_next_subnode(ofnode node)
{
	...
	return offset_to_ofnode(
		fdt_next_subnode(dm_fdt_blob(), ofnode_to_offset(node)));
}
...
ofnode ofnode_get_parent(ofnode node)
{
	...
	if (ofnode_is_np(node))
		...
		parent.of_offset = fdt_parent_offset(dm_fdt_blob(),
						     ofnode_to_offset(node));
...
const char *ofnode_get_name(ofnode node)
{
	...
	return fdt_get_name(dm_fdt_blob(), ofnode_to_offset(node), NULL);
}
...
ofnode ofnode_get_by_phandle(uint phandle)
{
	...
	if (of_live_active())
		...
		node.of_offset = fdt_node_offset_by_phandle(dm_fdt_blob(),
							    phandle);
...
fdt_addr_t ofnode_get_addr_size_index(ofnode node, int index, fdt_size_t *size)
{
	...
	*size = FDT_SIZE_T_NONE;
	...
	if (ofnode_is_np(node)) {
		...
		return fdtdec_get_addr_size_fixed(dm_fdt_blob(),
			     ...
			     const char *string)
{
	if (ofnode_is_np(node)) {
		...
		ret = fdt_stringlist_search(dm_fdt_blob(),
			     ...
			     const char **outp)
{
	if (ofnode_is_np(node)) {
		...
		*outp = fdt_stringlist_get(dm_fdt_blob(), ofnode_to_offset(node),
					   property, index, &len);
		if (len < 0)
			return -EINVAL;
		return 0;
...
int ofnode_read_string_count(ofnode node, const char *property)
{
	if (ofnode_is_np(node)) {
		...
		return fdt_stringlist_count(dm_fdt_blob(),
					    ofnode_to_offset(node), property);
	}
}
				   ...
				   struct ofnode_phandle_args *out_args)
{
	if (ofnode_is_np(node)) {
		...
		ret = fdtdec_parse_phandle_with_args(dm_fdt_blob(),
						     ofnode_to_offset(node),
						     list_name, cells_name,
				   ...
				   const char *cells_name, int cell_count)
{
	if (ofnode_is_np(node))
		...
		return fdtdec_parse_phandle_with_args(dm_fdt_blob(),
...
ofnode ofnode_path(const char *path)
{
	if (of_live_active())
		...
		return offset_to_ofnode(fdt_path_offset(dm_fdt_blob(), path));
}
...
const void *ofnode_get_property(ofnode node, const char *propname, int *lenp)
{
	if (ofnode_is_np(node))
		...
		return fdt_getprop(dm_fdt_blob(), ofnode_to_offset(node),
				   propname, lenp);
}
...
int ofnode_get_first_property(ofnode node, struct ofprop *prop)
{
	...
	if (ofnode_is_np(node)) {
		...
		prop->offset =
			fdt_first_property_offset(dm_fdt_blob(),
...
int ofnode_get_next_property(struct ofprop *prop)
{
	if (ofnode_is_np(prop->node)) {
		...
		prop->offset = fdt_next_property_offset(dm_fdt_blob(),
							prop->offset);
		if (prop->offset  < 0)
			return prop->offset;
	}
					...
					const char **propname, int *lenp)
{
	if (ofnode_is_np(prop->node))
		...
		return fdt_getprop_by_offset(dm_fdt_blob(),
					     prop->offset,
					     propname, lenp);
}
				...
				fdt_size_t *sizep)
{
	if (ofnode_is_np(node)) {
		...
		*sizep = of_read_number(prop + na, ns);
		...
		return fdtdec_get_addr_size(dm_fdt_blob(),
					    ofnode_to_offset(node), property,
					    sizep);
	}
}
					...
					size_t sz)
{
	if (ofnode_is_np(node)) {
		...
		return fdtdec_locate_byte_array(dm_fdt_blob(),
...
int ofnode_read_addr_cells(ofnode node)
{
	if (ofnode_is_np(node)) {
		...
		int parent = fdt_parent_offset(dm_fdt_blob(),
					       ofnode_to_offset(node));

		return fdt_address_cells(dm_fdt_blob(), parent);
	}
}
...
int ofnode_read_size_cells(ofnode node)
{
	if (ofnode_is_np(node)) {
		...
		return fdt_size_cells(dm_fdt_blob(), parent);
	}
}
...
int ofnode_read_simple_addr_cells(ofnode node)
{
	if (ofnode_is_np(node))
		...
		return fdt_address_cells(dm_fdt_blob(), ofnode_to_offset(node));
}
...
int ofnode_read_simple_size_cells(ofnode node)
{
	if (ofnode_is_np(node))
		...
		return fdt_size_cells(dm_fdt_blob(), ofnode_to_offset(node));
}
...
int ofnode_read_resource(ofnode node, uint index, struct resource *res)
{
	if (ofnode_is_np(node)) {
		...
		ret = fdt_get_resource(dm_fdt_blob(), ofnode_to_offset(node),
				       "reg", index, &fres);
		if (ret < 0)
			return -EINVAL;
...
u64 ofnode_translate_address(ofnode node, const fdt32_t *in_addr)
{
	if (ofnode_is_np(node))
		...
		return fdt_translate_address(dm_fdt_blob(), ofnode_to_offset(node), in_addr);
}
...
u64 ofnode_translate_dma_address(ofnode node, const fdt32_t *in_addr)
{
	if (ofnode_is_np(node))
		...
		return fdt_translate_dma_address(dm_fdt_blob(), ofnode_to_offset(node), in_addr);
}
...
int ofnode_get_dma_range(ofnode node, phys_addr_t *cpu, dma_addr_t *bus, u64 *size)
{
	if (ofnode_is_np(node))
		...
		return fdt_get_dma_range(dm_fdt_blob(), ofnode_to_offset(node),
					 cpu, bus, size);
}
...
int ofnode_device_is_compatible(ofnode node, const char *compat)
{
	if (ofnode_is_np(node))
		...
		return !fdt_node_check_compatible(dm_fdt_blob(),
						  ofnode_to_offset(node),
						  compat);
}
...
ofnode ofnode_by_compatible(ofnode from, const char *compat)
{
	if (of_live_active()) {
		...
		return offset_to_ofnode(fdt_node_offset_by_compatible(
				dm_fdt_blob(), ofnode_to_offset(from), compat));
	}
}
			    ...
			    const void *propval, int proplen)
{
	if (of_live_active()) {
		...
		return offset_to_ofnode(fdt_node_offset_by_prop_value(
				dm_fdt_blob(), ofnode_to_offset(from),
		      ...
		      const void *value)
{
	...
	new = rtems_malloc(sizeof(struct property));
	if (!new)
		return -ENOMEM;
