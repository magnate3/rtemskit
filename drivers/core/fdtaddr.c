...
fdt_addr_t devfdt_get_addr_index(const struct udevice *dev, int index)
{
	...
	if (CONFIG_IS_ENABLED(OF_TRANSLATE)) {
		...
		na = fdt_address_cells(dm_fdt_blob(),
		...
		ns = fdt_size_cells(dm_fdt_blob(), dev_of_offset(dev->parent));
		if (ns < 0) {
			debug("bad #size-cells\n");
			return FDT_ADDR_T_NONE;
		}

		reg = fdt_getprop(dm_fdt_blob(), dev_of_offset(dev), "reg",
				  &len);
		...
		if (ns) {
			 ...
			 * bus setups.
			 */
			addr = fdt_translate_address((void *)dm_fdt_blob(),
		 ...
		 * bus setups.
		 */
		addr = fdtdec_get_addr_size_auto_parent(dm_fdt_blob(),
				      ...
				      fdt_size_t *size)
{
	 ...
	 * all config options.
	 */
	fdtdec_get_addr_size_auto_noparent(dm_fdt_blob(), dev_of_offset(dev),
...
fdt_addr_t devfdt_get_addr_name(const struct udevice *dev, const char *name)
{
	...
	index = fdt_stringlist_search(dm_fdt_blob(), dev_of_offset(dev),
				      "reg-names", name);
	if (index < 0)
		return index;
