...
static rtems_mutex of_mutex = RTEMS_MUTEX_INITIALIZER("of-mutex");
...
int of_alias_scan(void)
{
	...
	for_each_property_of_node(of_aliases, pp) {
		...
		ap = rtems_malloc(sizeof(*ap) + len + 1);
		if (!ap)
			return -ENOMEM;
