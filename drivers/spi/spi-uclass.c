		...
		const void *dout, void *din, unsigned long flags)
{
	...
	rtems_mutex_lock(&spi->lock);
	ret = ops->xfer(dev, bitlen, dout, din, flags);
	rtems_mutex_unlock(&spi->lock);
	return ret;
}
		    ...
		    uint *offsetp)
{
	...
	rtems_mutex_lock(&spi->lock);
	ret = ops->get_mmap(dev, map_basep, map_sizep, offsetp);
	rtems_mutex_unlock(&spi->lock);
	return ret;
}
...
static int spi_post_probe(struct udevice *bus)
{
	...
	rtems_mutex_init(&spi->lock, bus->name);
...
static int spi_child_pre_probe(struct udevice *dev)
{
	...
	rtems_mutex_init(&slave->lock, dev->name);
	return 0;
}
