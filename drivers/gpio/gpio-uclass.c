...
#define GPIO_LOCK()   rtems_mutex_lock(&gpio_lock)
#define GPIO_UNLOCK() rtems_mutex_unlock(&gpio_lock)


static rtems_mutex gpio_lock = RTEMS_MUTEX_INITIALIZER("gpio");

static void gpio_desc_init(struct gpio_desc *desc,
	struct udevice *dev, uint offset) 
{
	desc->dev = dev;
	desc->offset = offset;
	desc->flags = 0;
...
int dm_gpio_get_value_nocheck(const struct gpio_desc *desc)
{
	const struct dm_gpio_ops *ops = gpio_get_ops(desc->dev);
	int value;
...
int dm_gpio_get_value(const struct gpio_desc *desc)
{
	...
	if (ret)
		return ret;
	return dm_gpio_get_value_nocheck(desc);
}

int dm_gpio_set_value_nocheck(const struct gpio_desc *desc, int value)
{
	const struct dm_gpio_ops *ops = gpio_get_ops(desc->dev);
	...
	return ops->set_value(desc->dev, desc->offset, value);
}

int dm_gpio_set_value(const struct gpio_desc *desc, int value)
{
	int ret = check_reserved(desc, "set_value");
	if (ret)
		return ret;
	return dm_gpio_set_value_nocheck(desc, value);
}
...
int dm_gpio_get_flags(struct gpio_desc *desc, ulong *flagsp)
{
	...
	if (ops->get_flags) {
		...
		flags &= ~GPIOD_IS_OUT_ACTIVE;
		if ((desc->flags & GPIOD_IS_OUT) && dm_gpio_get_value_nocheck(desc))
			flags |= GPIOD_IS_OUT_ACTIVE;
	}
	*flagsp = flags;
	return 0;
...
static int gpio_post_probe(struct udevice *dev)
{
	...
	ptr = calloc(uc_priv->gpio_count, 
        sizeof(char *) + sizeof(struct gpio_irqdesc *));
	if (!ptr)
		return -ENOMEM;
    uc_priv->name = (char **)ptr;
    uc_priv->irqdescs = (struct gpio_irqdesc **)(ptr + 
		uc_priv->gpio_count * sizeof(char *));
	rtems_mutex_init(&uc_priv->irqlock, uc_priv->irqchip->name);
	rtems_interrupt_lock_initialize(&uc_priv->lock, uc_priv->bank_name);
...
int gpiod_request_irq(struct gpio_desc *desc, int (*handler)(void *), 
   void *arg, unsigned int flags)
{
   struct gpio_dev_priv *uc_priv = dev_get_uclass_priv(desc->dev);
   struct gpio_irqdesc *irqdesc, **iter;
   int ret;
   ...
   rtems_mutex_lock(&uc_priv->irqlock);
   iter = &uc_priv->irqdescs[desc->offset];
   while (*iter) {
       if ((*iter)->handler == handler && (*iter)->arg == arg) {
           ret = -EEXIST;
           goto _out;
       }
       iter = &(*iter)->next;
   }
   irqdesc = rtems_malloc(sizeof(struct gpio_irqdesc));
   if (!irqdesc) {
       ret = -ENOMEM;
       goto _out;
   }
   irqdesc->handler = handler;
   irqdesc->arg = arg;
   irqdesc->next = NULL;
   *iter = irqdesc;
   ret = uc_priv->irqchip->irq_set_type(desc, 
       flags & IRQ_TYPE_SENSE_MASK);
   if (ret)
       goto _freem;
   uc_priv->irqchip->irq_unmask(desc);
   goto _out;
...
_freem:
   *iter = NULL;
   free(irqdesc);
_out:
   rtems_mutex_unlock(&uc_priv->irqlock);
   return ret;
}
