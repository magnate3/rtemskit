			     ...
			     struct reset_ctl *reset_ctl)
{
	...
	ret = ofnode_parse_phandle_with_args(node, "resets", 
		"#reset-cells", 0, index, &args);
			    ...
			    struct reset_ctl_bulk *bulk)
{
	...
	bulk->count = 0;
	count = ofnode_count_phandle_with_args(node, "resets", 
		"#reset-cells", 0);
	if (count < 1)
		return count;
...
{
	struct reset_uclass *uc = dev_get_uclass_priv(dev);
	int ret;

	rtems_mutex_lock(&uc->lock);
	ret = __reset_get_bulk(dev, dev_ofnode(dev), bulk);
	rtems_mutex_unlock(&uc->lock);
	return ret;
}
		     ...
		     struct reset_ctl *reset_ctl)
{
	...
	ret = reset_get_by_index(dev, index, reset_ctl);
...
int reset_request(struct reset_ctl *reset_ctl)
{
	...
	struct reset_uclass *uc = dev_get_uclass_priv(reset_ctl->dev);
	int ret;
...
int reset_deassert(struct reset_ctl *reset_ctl)
{
	...
	debug("%s(reset_ctl=%p)\n", __func__, reset_ctl);
	rtems_mutex_lock(&uc->lock);
	ret = ops->rst_deassert(reset_ctl);
	rtems_mutex_unlock(&uc->lock);
	return ret;
}
						   ...
						   ofnode node)
{
	...
	bulk = devres_alloc(devm_reset_bulk_release,
			    sizeof(struct reset_ctl_bulk), __GFP_ZERO);
	if (unlikely(!bulk))
		return ERR_PTR(-ENOMEM);
...
static int reset_post_probe(struct udevice *dev)
{
	struct reset_uclass *uc = dev_get_uclass_priv(dev);

	rtems_mutex_init(&uc->lock, dev->name);
	return 0;
}
...
UCLASS_DRIVER(reset) = {
	.id		= UCLASS_RESET,
	.name	= "reset",
	.post_probe = reset_post_probe,
	.per_device_auto = sizeof(struct reset_uclass)
};
