...
{
  pthread_once_t        once;
  rtems_recursive_mutex lock;           /**< The RAP lock id */
...
rtems_rap_data_init (void)
{
   ...
   * Create the RAP lock.
   */
  rtems_recursive_mutex_init (&rap_.lock, "RAP");
...
rtems_rap_lock (void)
{
  ...
  rtems_recursive_mutex_lock (&rap_.lock);
...
{
  rtems_recursive_mutex_unlock (&rap_.lock);
}
...
static void*
__rtems_rap_find (const char* name, rtems_rap_data *rap)
{
  rtems_chain_node* node;
...
{
  rtems_rap_data*   rap = rtems_rap_lock ();
  rtems_rap_app* app = __rtems_rap_find (name, rap);
  rtems_rap_unlock ();
  return app;
}
